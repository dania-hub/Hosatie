...
const fetchData = async () => {
    ...
    error.value = null;
    try {
        const response = await api.get(`/super-admin/suppliers/${props.supplier.id}/deactivation-data`);
        data.value = response.data.data;
        
        // Default target supplier (first in list)
        if (data.value.alternativeSuppliers && data.value.alternativeSuppliers.length > 0) {
            formData.value.target_supplier_id = data.value.alternativeSuppliers[0].id;
        }
    } catch (err) {
...
const finalizeDeactivation = async () => {
    ...
    error.value = null;
    try {
        const response = await api.patch(`/super-admin/suppliers/${props.supplier.id}/deactivate`, formData.value);
...
const canMoveToNext = computed(() => {
    ...
    if (step.value === 2) return formData.value.target_supplier_id !== null;
    return true;
});
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] rtl">
            ...
            <div class="bg-[#2E5077] p-5 text-white flex justify-between items-center">
                <div class="flex items-center gap-3">
                    <div class="p-2 bg-white/10 rounded-lg">
                        <Icon icon="solar:box-broken" class="w-6 h-6" />
                    </div>
                    <div>
                        <h3 class="text-xl font-bold">معالج إيقاف تفعيل المورد (المستودع)</h3>
                        <p class="text-xs text-blue-100 opacity-80">{{ props.supplier.name }}</p>
                    </div>
                </div>
            ...
            <div class="flex-1 overflow-y-auto p-6 bg-white min-h-[300px]">
                ...
                <div v-if="loading" class="flex flex-col items-center justify-center h-full py-10">
                    ...
                    <p class="text-gray-600">جاري فحص حالة المورد والعمليات المرتبطة...</p>
                </div>
                ...
                <div v-else-if="step === 1" class="space-y-6 animate-fade-in">
                    <div v-if="data.hasBlockers" class="bg-amber-50 border border-amber-200 p-5 rounded-xl">
                        ...
                        <p class="mt-4 text-sm text-gray-600">يجب إكمال أو إلغاء هذه الطلبات قبل التمكن من إيقاف تفعيل المورد.</p>
                    </div>
                    ...
                    <div v-else>
                        ...
                        <p class="text-gray-600 mb-6 text-sm">تم التحقق من جاهزية المورد للإيقاف. الكيانات التالية سيتم التعامل معها:</p>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="bg-blue-50 p-4 rounded-xl border border-blue-100 flex items-center gap-4">
                                <div class="p-3 bg-blue-500/10 rounded-lg text-blue-600">
                                    <Icon icon="solar:hospital-bold" class="w-6 h-6" />
                                </div>
                                <div>
                                    <p class="text-2xl font-bold text-blue-700">{{ data.counts.hospitals }}</p>
                                    <p class="text-xs text-blue-600 uppercase font-bold">مستشفى تابعة</p>
                                </div>
                            </div>
                            <div class="bg-orange-50 p-4 rounded-xl border border-orange-100 flex items-center gap-4">
                                ...
                                <div>
                                    <p class="text-2xl font-bold text-orange-700">{{ data.counts.inventoryItems }}</p>
                                    <p class="text-xs text-orange-600 uppercase font-bold">صنف دواء ذو رصيد</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Step 2: Relink & Inventory -->
                <div v-else-if="step === 2" class="space-y-8 animate-fade-in">
                    <!-- Relink Hospitals -->
                    <div class="space-y-4">
                        <div class="flex items-center gap-3 text-[#2E5077]">
                            <Icon icon="solar:link-break-bold" class="w-6 h-6" />
                            <h4 class="font-bold">إعادة ربط المستشفيات التابعة ({{ data.counts.hospitals }})</h4>
                        </div>
                        <div class="form-control w-full">
                            <label class="label">
                                <span class="label-text font-bold">اختر المورد البديل الذي ستتبع له هذه المستشفيات:</span>
                            </label>
                            <select v-model="formData.target_supplier_id" class="select select-bordered w-full rounded-xl bg-gray-50 focus:border-[#4DA1A9]">
                                <option disabled :value="null">اختر مورد بديل...</option>
                                <option v-for="s in data.alternativeSuppliers" :key="s.id" :value="s.id">
                                    {{ s.name }} ({{ s.city }})
                                </option>
                            </select>
                        </div>
                    ...
                    <div class="space-y-4 border-t pt-6">
                        <div class="flex items-center gap-3 text-[#2E5077]">
                            <Icon icon="solar:transfer-vertical-bold" class="w-6 h-6" />
                            <h4 class="font-bold">نقل المخزون</h4>
                        </div>
                        ...
                        <div class="flex flex-col gap-3">
                            <label class="flex items-center gap-3 bg-gray-50 p-4 rounded-xl border border-gray-100 cursor-pointer hover:bg-gray-100 transition-colors">
                                <input type="checkbox" v-model="formData.transfer_inventory" class="checkbox checkbox-primary" />
                                <div>
                                    <p class="font-bold text-sm">نقل أرصدة المخزون الحالية إلى المورد البديل</p>
                                    <p class="text-xs text-gray-500">سيتم تصفير الكميات في المورد الحالي وإضافتها إلى المورد المختار أعلاه.</p>
                                </div>
                            </label>
                            <p v-if="!formData.transfer_inventory" class="text-xs text-amber-600 mr-2">
                                * في حالة عدم النقل، ستبقى الأرصدة مسجلة تاريخياً ولكنها غير متاحة للطلب.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div v-else-if="step === 3" class="space-y-6 animate-fade-in">
                    <div class="flex items-center gap-3 text-[#2E5077] mb-4">
                        ...
                        <h4 class="text-lg font-bold">التعامل مع حساب المسؤول</h4>
                    </div>

                    <div v-if="data.manager" class="bg-gray-50 p-5 rounded-2xl border border-gray-200">
                        <div class="flex items-center gap-3 mb-4">
                            <div class="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold">
                                {{ data.manager.name.charAt(0) }}
                            </div>
                            <div>
                                <p class="font-bold text-gray-800">{{ data.manager.name }}</p>
                                <p class="text-xs text-gray-500">مدير المورد</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            <label class="flex items-center gap-2 text-sm cursor-pointer">
                                ...
                                <span>إيقاف تفعيل الحساب (Deactivate Account)</span>
                            </label>
                        </div>
                    </div>
                    <div v-else class="text-center py-6 text-gray-500 italic">
                        لا يوجد مدير مرتبط بهذا المورد حالياً.
                    </div>

                    <div class="bg-blue-50 p-4 rounded-xl border border-blue-100">
                        <p class="text-xs text-blue-800 leading-relaxed">
                            <Icon icon="tabler:info-circle" class="inline w-3 h-3 ml-1" />
                            <strong>الإيقاف النهائي:</strong> بعد هذه الخطوة، سيتم تغيير حالة المورد إلى Inactive ولن يظهر في قائمة الخيارات عند تقديم طلبات توريد جديدة.
                        </p>
                    </div>
                </div>
