...
const stats = ref({
    ...
    todayRegistered: 0,
    weekRegistered: 0,
    totalSupplyRequests: 0,
    completedReceipts: 0,
    isLoading: true,
    error: null
});

// إحصائيات طلبات التوريد
...
const fetchStats = async () => {
    ...
    try {
        const response = await api.get('/department-admin/dashboard/stats');
...
// جلب إحصائيات طلبات التوريد
const fetchSupplyStats = async () => {
    supplyStats.value.isLoading = true;
    try {
        // جلب جميع الشحنات (طلبات التوريد)
        const response = await api.get('/department-admin/shipments');
        ...
        stats.value.totalSupplyRequests = 0;
        stats.value.completedReceipts = 0;
    } finally {
        supplyStats.value.isLoading = false;
    }
};
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="card bg-white p-6 rounded-2xl shadow-lg border-2 border-[#2E5077] flex flex-col transition-all duration-300 hover:shadow-xl hover:-translate-y-1" dir="rtl">
                        <div class="content flex items-center gap-3 mb-4" style="justify-content: flex-start;">
                            ...
                            <p class="text text-lg font-bold text-[#2E5077]" style="text-align: right;">عدد المرضى اليوم</p>
                        </div>
                    ...
                    <div class="card bg-white p-6 rounded-2xl shadow-lg border-2 border-[#4DA1A9] flex flex-col transition-all duration-300 hover:shadow-xl hover:-translate-y-1" dir="rtl">
                        <div class="content flex items-center gap-3 mb-4" style="justify-content: flex-start;">
                            ...
                            <p class="text text-lg font-bold text-[#4DA1A9]" style="text-align: right;">عدد الكشوفات اليوم</p>
                        </div>
                    ...
                    <div class="card bg-white p-6 rounded-2xl shadow-lg border-2 border-[#79D7BE] flex flex-col transition-all duration-300 hover:shadow-xl hover:-translate-y-1" dir="rtl">
                        <div class="content flex items-center gap-3 mb-4" style="justify-content: flex-start;">
                            ...
                            <p class="text text-lg font-bold text-[#79D7BE]" style="text-align: right;">عدد الحالات التي تتابعها</p>
                        </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="card bg-white p-6 rounded-2xl shadow-lg border-2 border-[#2E5077] flex flex-col transition-all duration-300 hover:shadow-xl hover:-translate-y-1" dir="rtl">
                        ...
                        <p class="number text-5xl font-bold text-[#2E5077]" style="text-align: right; width: 100%;">
                            <span v-if="supplyStats.isLoading">...</span>
                            <span v-else>{{ stats.totalSupplyRequests }}</span>
                        </p>
                    </div>
                    ...
                    <div class="card bg-white p-6 rounded-2xl shadow-lg border-2 border-[#4DA1A9] flex flex-col transition-all duration-300 hover:shadow-xl hover:-translate-y-1" dir="rtl">
                        ...
                        <p class="number text-5xl font-bold text-[#4DA1A9]" style="text-align: right; width: 100%;">
                            <span v-if="supplyStats.isLoading">...</span>
                            <span v-else>{{ stats.completedReceipts }}</span>
                        </p>
                    </div>
                </div>
            </div>
