...
const fetchInventory = async () => {
    ...
    try {
        ...
        console.error('فشل جلب مخزون المستشفيات:', err);
        error.value = "حدث خطأ أثناء تحميل بيانات مخزون المستشفيات.";
    } finally {
...
<head>
    <meta charset="UTF-8">
    <title>تقرير مخزون المستشفيات - ${currentDate}</title>
    ...
    <div class="print-container">
        <div class="page-header">
            ...
            <div style="text-align: left;">
                ...
                <p style="margin: 5px 0; color: #64748b; font-size: 12px;">رمز التقرير: HOSPITAL/INV/${new Date().getTime().toString().slice(-6)}</p>
            </div>
        </div>

        <div class="report-title">
            <h1>تقرير مخزون المستشفيات العام</h1>
        </div>
                            ...
                            <tbody class="text-gray-800">
                                ...
                                <template v-else>
                                                ...
                                                <div class="px-12 py-4 border-b border-gray-100">
                                                    <table class="w-full text-sm">
                                                        <thead class="text-gray-500 border-b border-gray-200">
                                                            <tr>
                                                                <th class="py-2 text-right">الجهة (مخزن/صيدلية)</th>
                                                                <th class="py-2 text-right">الكمية المتوفرة</th>
                                                                <th class="py-2 text-right">إجمالي الطلبات المعلقة</th>
                                    ...
                                    <tr v-if="filteredInventories.length === 0">
                                        <td colspan="5" class="py-12"><EmptyState message="لا توجد بيانات مخزون للمستشفيات" /></td>
                                    </tr>
                                </template>
                            </tbody>
                        </table>
