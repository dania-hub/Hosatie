...
const API_ENDPOINTS = {
    shipments: {
        ...
        receive: (id, data) => api.put(`/shipments/${id}/receive`, data)
    },
    categories: {
        getAll: () => api.get('/categories') // Assuming /super-admin/categories or /api/categories?
        // Route list showed Route::get('categories', ...) in super-admin group?
        // No, it showed drugs, hospitals, suppliers...
        // But maybe categories are generic?
        // Let's assume it works or returns empty.
    },
    drugs: {
        getAll: () => api.get('/drugs'), // /api/super-admin/drugs exists
        getAllDrugs: () => api.get('/drugs'),
        getByCategory: (categoryId) => api.get(`/drugs?categoryId=${categoryId}`)
    },
    departments: {
        getAll: () => Promise.resolve({ data: [] }) // SuperAdmin might not need this here
    }
};
...
const fetchShipments = async () => {
    try {
        ...
        // Laravel wraps data in 'data' property usually
        const shipmentsArray = response.data || response;
...
const fetchDrugs = async () => {
    try {
        ...
        allDrugsData.value = drugs.map(drug => ({
            id: drug.id,
            name: drug.name,
            categoryId: drug.categoryId || drug.category,
...
const openRequestViewModal = async (shipment) => {
    try {
        // Use details directly from the shipment object if available
        // to avoid unnecessary API calls that might fail or return incomplete data
        const response = shipment.details || await API_ENDPOINTS.shipments.getById(shipment.id);
        ...
        selectedRequestDetails.value = {
            ...
            items: (response.items || []).map(item => ({
                ...
                quantity: item.requestedQty || item.quantity || item.requested_qty || 0,
                requested_qty: item.requestedQty || item.requested_qty || 0,
...
const openConfirmationModal = async (shipment) => {
    ...
    try {
        ...
        let response = await API_ENDPOINTS.shipments.getById(shipment.id);
        // Correctly unwrap the response from generic API wrapper
        ...
        selectedShipmentForConfirmation.value = {
            ...
            items: (response.items || []).map(item => {
                ...
                return {
                    ...
                    quantity: item.requestedQty || item.quantity || item.requested_qty || 0,
                    requested_qty: item.requestedQty || item.requested_qty || 0,
                    requestedQuantity: item.requestedQty || item.requested_qty || 0,
                    originalQuantity: item.requestedQty || item.requested_qty || 0,
                    availableQuantity: item.availableQuantity !== undefined && item.availableQuantity !== null ? item.availableQuantity : (item.stock !== undefined && item.stock !== null ? item.stock : 0),
    ...
    try {
        if (confirmationData.rejectionReason) {
            ...
            const requestData = {
                ...
                confirmedBy: 'مدير النظام' 
            };
            
            console.log('Request data:', requestData);
            
            // Depending on endpoint, confirm might be for receiving or sending. 
            // If SuperAdmin is fulfilling requests (Sending), we use confirm with sent items.
            ...
            if (shipmentIndex !== -1) {
                // Update local status
                shipmentsData.value[shipmentIndex].requestStatus = 'قيد الاستلام'; // Or 'Sent' ?
                    ...
                    shipmentsData.value[shipmentIndex].details.items = 
                        shipmentsData.value[shipmentIndex].details.items.map(item => {
                            ...
                            if (sentItem) {
                                return { 
                                    ...item, 
                                    approved_qty: sentItem.sentQuantity, 
                                    sentQuantity: sentItem.sentQuantity 
                                };
                            }
                            return item;
                        });
            ...
            showSuccessAlert(` تم تأكيد إرسال الشحنة رقم ${shipmentNumber} بنجاح! (${totalSent} وحدة)`);
        }
