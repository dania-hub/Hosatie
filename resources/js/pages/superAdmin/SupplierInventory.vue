...
const printTable = () => {
    ...
    printWindow.document.write('<html><head><title>طباعة مخزون الموردين</title></head><body>');
                                ...
                                <template v-else>
                                    <tr v-for="item in filteredInventories" :key="item.id" class="hover:bg-gray-100 border border-gray-300">
                                        ...
                                        <td class="p-4 font-bold text-blue-600">
                                            <div v-if="item.units_per_box > 1">
                                                {{ item.current_quantity_boxes }} {{ item.unit === 'مل' ? 'عبوة' : 'علبة' }}
                                                <span v-if="item.current_quantity_remainder > 0" class="text-xs text-gray-400 font-normal">
                                                    و {{ item.current_quantity_remainder }} {{ item.unit === 'مل' ? 'مل' : 'حبة' }}
                                                </span>
                                            </div>
                                            <div v-else>
                                                {{ item.current_quantity }} {{ item.unit }}
                                        ...
                                        <td class="p-4 font-bold text-red-600">
                                            {{ item.needed_quantity_boxes }} {{ item.unit === 'مل' ? 'عبوة' : 'علبة' }}
                                        </td>
